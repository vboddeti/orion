...
Tests parallel execution with a minimal model to verify that Option 2
(Backend Evaluator Pool) works correctly.
"""
import torch
import orion
...
import numpy as np
import time
# TEMP: Comment out to test if this is causing issues
...
def test_parallel_simple():
    """
    Test parallel execution with a simple 2-layer model.

    Creates 4 independent inputs and processes them in parallel.
    """
    print("\n" + "="*70)
    ...
    # Fit and compile
    print("Step 2: Running orion.fit() and compile()...")
    orion.fit(model, inp)

    # DEBUG: Check evaluator type before compile
    from orion.core import scheme
    print(f"DEBUG: Evaluator type before compile: {type(scheme.evaluator).__name__}")
    print(f"DEBUG: LT Evaluator type before compile: {type(scheme.lt_evaluator).__name__}")

    input_level = orion.compile(model)
    print(f"  ✓ Compilation successful! Input level: {input_level}")

    # DEBUG: Check evaluator type after compile
    print(f"DEBUG: Evaluator type after compile: {type(scheme.evaluator).__name__}")
    print(f"DEBUG: LT Evaluator type after compile: {type(scheme.lt_evaluator).__name__}")

    # Create 4 different inputs for parallel processing
    print("\nStep 3: Creating 4 test inputs...")
    test_inputs = []
    cleartext_outputs = []
    ...
    # Enable thread-safe mode RIGHT BEFORE parallel processing
    print("\nStep 5: Enabling thread-safe mode for parallel processing...")
    orion.enable_thread_safe_mode()

    # TEMP: Process sequentially to test if ThreadPoolExecutor is the issue
    print(f"\nStep 6: Processing {len(encrypted_inputs)} inputs SEQUENTIALLY (temp)...")
    ...
    print(f"\nEstimated sequential time: ~{parallel_time * len(encrypted_inputs):.2f} seconds")
    print(f"Theoretical speedup: ~{len(encrypted_inputs):.1f}x")
